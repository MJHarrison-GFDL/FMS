...
!</NAMELIST>
  ...
  use mpp_io_mod, only : mpp_open, mpp_get_atts, mpp_get_info, MPP_NETCDF, MPP_MULTI, MPP_SINGLE,&
       ...
       mpp_get_tavg_info, validtype, mpp_is_valid, mpp_get_file_name, mpp_get_time_value
  use time_manager_mod, only : time_type, get_date, set_date, operator ( >= ) , operator ( + ) , days_in_month, &
                            ...
                            set_time, get_time, operator( > ), operator( < ), operator( <= ), get_calendar_type, NO_CALENDAR
...
#include<file_version.h>
  ...
  type, private :: ext_fieldtype
     ...
     character(len=128) :: calendar_type, time_units
     logical :: permit_calendar_conversion
...
!               data(isw:iew,jsw:jew,:) = time_interp_missing !field(index)%missing? Balaji
        ...
        if(field(index)%have_modulo_times) then
          ...
          call adjust_time_weights(time,index,t1,t2,w2) ! may need to adjust interpolation weights
                                      ! if using estimated time axis. This involves reading time record values.
          if(err_msg .NE. '') then
             filename = mpp_get_file_name(field(index)%unit)
...
! ===========================================================================
subroutine adjust_time_weights(time,index, i1, i2, w2)
  type(time_type), intent(in) :: time
  integer        , intent(in)  :: index
  integer, intent(inout) :: i1, i2 ! indices
  real, intent(inout)  ::  w2
  integer :: unit, nt
  integer :: ndim, nvar, ncatt, ntime, natt
  type(axistype), dimension(:), allocatable :: axes
  type(axistype) :: time_axis
  real :: t1_time, t2_time
  type(time_type) :: time1, time2
  character(len=128) :: units, calendar_type
  logical :: permit_calendar_conversion


  unit=field(index)%unit
  call mpp_get_info(unit,ndim,nvar,natt,ntime)
  allocate(axes(ndim))
  call mpp_get_axes(unit, axes, time_axis)
  call mpp_get_atts(time_axis,units=units, calendar=calendar_type)
  permit_calendar_conversion=field(index)%permit_calendar_conversion
  t1_time=mpp_get_time_value(unit, i1)
  t2_time=mpp_get_time_value(unit, i2)
  time1=get_cal_time(t1_time,trim(field(index)%time_units),trim(calendar_type),permit_calendar_conversion)
  time2=get_cal_time(t2_time,trim(field(index)%time_units),trim(calendar_type),permit_calendar_conversion)
  if (time<time1) then
      do nt=i1-1,1,-1
        t1_time=mpp_get_time_value(field(index)%unit, nt)
        time1=get_cal_time(t1_time,trim(field(index)%time_units),trim(calendar_type),permit_calendar_conversion)
        if (time1<=time) then
            i1=nt
            exit
  ...
  if (time>time2) then
      do nt=i2+1,ntime
        t2_time=mpp_get_time_value(field(index)%unit, nt)
        time2=get_cal_time(t2_time,trim(field(index)%time_units),trim(calendar_type),permit_calendar_conversion)
        if (time2>=time) then
            i2=nt
            exit
  ...
  call time_interp(time,(/time1,time2/),w2,i1,i2)

end subroutine adjust_time_weights
